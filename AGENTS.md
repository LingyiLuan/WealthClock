...
| 级 | 谁审 | 合并 | 适用 |
|---|---|---|---|
| **L0** | 人类逐行 | 人类 | 引擎参数、金标固件、支付、CI/工作流、本文件 |
| **L1** | CI + 人类看 diff | 人类 | 新功能、新页面(默认级) |
| **L2** | CI + 议会评审 + 人类看证据(不看 diff) | 人类 | 已有页面的迭代、样式、重构 |
| **L3** | CI + 策略 | **自动合并** | 允许名单:`docs/**`(不含 adr)、`*.xcstrings` 本地化、`docs/design/**` |
