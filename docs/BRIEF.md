...
| 真实年化收益 | 3.5% | 5% | 7% | 历史真实回报区间;悲观与 3.5% 提取率同一套历史假设(ADR-0003),TODO(calibrate) |
...
| 峰值后收入实际增速 | 0% | 0% | 0% | 同上;悲观不再叠加收入下降(ADR-0003) |
| 支出实际增速 | 0% | 0% | 0% | 悲观不再叠加生活方式通胀(ADR-0003),`TODO(calibrate)` |

**学历修正**(作用于峰值前收入增速):belowBachelor −0.5pp,bachelor 0,masterPlus +0.5pp。出处:Mincer 教育回报;`TODO(calibrate)`。

**行业修正**(作用于峰值年龄):tech −2 岁(cnMainland 下再 −1,ADR-0003),government/healthcare/education +5 岁,其他 0。`TODO(calibrate)`,标注"行业曲线待接入国家统计局分行业工资数据"。
...
**Reveal(揭晓)**:eyebrow"你的财务自由年龄"→ 点阵大数字 → "岁" → 苏州码子行(〡〢〣〤〥〦〧〨〩 映射)→ 三枚铜钱 + 六爻 → 三情景 chip → 密押行(朱砂)→ 主按钮"查看完整推演"、次按钮"保存汇票"。**未购买用户也能看到三情景数字**,这是产品原则。freedomAge 为 nil 时显示"把储蓄率提到 X% 可在 65 岁前自由",X 由引擎 `requiredSavingsRate(for:)` 二分搜索给出(引擎与测试 M1 已落地,UI M7 实现;ADR-0003)。
